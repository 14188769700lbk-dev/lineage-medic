...
const evidence: EvidenceEvent[] = [
  ...
  },
  {
    ...
    actor: "DataHub fixture",
    action: "get_lineage replayed",
    detail: "5 downstream assets across dbt, Airflow, Looker and Power BI",
    tool: "get_lineage",
    status: "complete",
  },
  {
    ...
    actor: "DataHub fixture",
    action: "get_entities replayed",
    detail: "Resolved owners, domains, platforms and usage metadata for 6 assets",
    tool: "get_entities",
    status: "complete",
  },
  {
    ...
    actor: "DataHub fixture",
    action: "get_dataset_queries replayed",
    detail: "3 relevant production query examples retained in the fixture",
...
export function createHostedInitialCampaign(): RepairCampaign {
  ...
  return {
    ...
    execution: {
      ...campaign.execution,
      contextLabel: "Recorded DataHub MCP fixture · hosted replay",
    },
  };
}
